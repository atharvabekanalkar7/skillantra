...
export default function EditTaskPage() {
    ...
    const [taskStatus, setTaskStatus] = useState<'open' | 'closed'>('open');
    ...
    const loadTask = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                setPageError(data.error || 'Failed to load task');
                return;
            }
            ...
            if (profileRes.ok) {
                ...
                if (profileData.profile?.id !== task.creator_profile_id) {
                    setPageError('You are not authorized to edit this task');
                    return;
                }
            }
            ...
            setSkillsRequired(task.skills_required || '');
            setPaymentType(task.payment_type || '');
            setStipendMin(task.stipend_min !== null ? String(task.stipend_min) : '');
            setStipendMax(task.stipend_max !== null ? String(task.stipend_max) : '');
            setPaymentOtherDetails(task.payment_other_details || '');
            setModeOfWork(task.mode_of_work || '');
            setTaskStatus(task.status);

            // Set deadline — convert ISO to datetime-local format
            if (task.application_deadline) {
                const d = new Date(task.application_deadline);
                const local = new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
                setApplicationDeadline(local);
            }
    ...
    if (pageError) {
        return (
            <div className="opacity-0 animate-fade-in-up max-w-2xl mx-auto">
    ...
    return (
        <div className="max-w-2xl mx-auto opacity-0 animate-fade-in-up">
            <div className="mb-6 md:mb-8">
                ...
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2">Edit Task</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        {attachments.length === 0 && (
                            <p className="text-sm text-white/50">No attachments. Click "Add Attachment" to include links.</p>
                        )}
