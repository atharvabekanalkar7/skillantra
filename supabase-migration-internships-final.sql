-- ============================================
-- Internships System Final Schema Setup
-- ============================================
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='internships' AND column_name='company_name') THEN
    ALTER TABLE internships ADD COLUMN company_name TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='internships' AND column_name='company_logo_url') THEN
    ALTER TABLE internships ADD COLUMN company_logo_url TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='internships' AND column_name='skills_required') THEN
    ALTER TABLE internships ADD COLUMN skills_required TEXT[];
  END IF;
